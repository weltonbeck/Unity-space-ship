...
public class Asteroid : MonoBehaviour {
	...
	void Dead () {
		...
		// chamo o elemento da cena GameController e envio uma mensagem pra ele
		GameObject gameController = GameObject.FindWithTag("GameController");
		gameController.SendMessage("SetScore",ScorePoints);
	...
	void OnTriggerEnter2D(Collider2D coll) {		
		if (coll.gameObject.tag == "Bullet"
		    || coll.gameObject.tag == "BulletEnemy") {
			// destruo a bala
